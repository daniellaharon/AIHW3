...
class ID3:
    ...
    def entropy(rows: np.ndarray, labels: np.ndarray):
        ...
        # ====== YOUR CODE: ======
        samples_size = sum(counts.values())
        for item in counts.values():
            impurity -= (item/samples_size * np.log2(item/samples_size))
        # ========================
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        info_gain_value = 0.0
        # ====== YOUR CODE: ======
        left_val = (len(left)/(len(left)+len(right)))
        right_val = (len(right)/(len(left)+len(right)))
        info_gain_value = current_uncertainty - (self.entropy(left, left_labels)*left_val) - (self.entropy(right, right_labels)*right_val)
        # ========================
    ...
    def find_best_split(self, rows, labels):
        ...
        for feature_idx in range(len(rows[0])):
            ...
            average_values = [(values[value]+values[value+1])/2 for value in range(len(values)-1)]
    ...
    def build_tree(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        #there is a single class ,no need to continue asking questions
        if len(class_counts(rows, labels)) == 1:
            return Leaf(rows, labels)
        #not enough rows to prun
    ...
    def predict_sample(self, row, node: DecisionNode or Leaf = None):
        ...
        :param row: vector of shape (1,D). (1 is rows, D is columns)
        ...
        # ====== YOUR CODE: ======
        #Decide whether to follow the true-branch or the false-branch.
        while type(node) is DecisionNode:
            if node.question.match(row):
                node = node.true_branch
            else:
                node = node.false_branch
        #now were at a leaf , can use the predictions field (class->count dictionary)
        #Compare the feature / value stored in the node, to the example we're considering.

        max_val = - math.inf
        for class_type, num in node.predictions.items():
            if num > max_val:
                max_val = num
    ...
    def predict(self, rows):
        ...
        # ====== YOUR CODE: ======
        y_pred = np.array([self.predict_sample(r) for r in rows])
        # ========================
