...
ID = 209149772  # TODO: change it to your personal ID
...
def accuracy(y: np.array, y_pred: np.array):
    ...
    # ====== YOUR CODE: ======
    #y is ground truth and y pred is the predictions
    accuracy_val = np.sum(y == y_pred)
    accuracy_val = accuracy_val/len(y)
    # ========================
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    # ====== YOUR CODE: ======
    dists = np.array([[np.linalg.norm(i-j) for j in x2] for i in x1])
    # ========================
